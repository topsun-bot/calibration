...
def calibrate_fused(
    ...
    """
    运行多种 hand-eye 算法，按一致性残差加权融合。

    权重: w_i ∝ 1 / (residual_mean_i + eps)^2
    旋转: 四元数 Markley 加权平均
    平移: 加权平均
    """
    # 退化检测
    ...
    for name in names:
        try:
            R, t = calibrate_single(R_a_list, t_a_list, R_t2c_list, t_t2c_list, name)
    ...
    eps = 1e-9
    inv_var = np.array([1.0 / (r.residual_mean + eps) ** 2 for r in results])
    inv_var = np.maximum(inv_var, min_weight)
    ...
    if mean_r > best.residual_mean:
        print(
            f"[融合] 加权融合残差 ({mean_r:.6f}m) 劣于最优单算法 {best.name} "
            f"({best.residual_mean:.6f}m)，采用 {best.name}"
        )
        R_f, t_f = best.R, best.t
        mean_r, max_r = best.residual_mean, best.residual_max
